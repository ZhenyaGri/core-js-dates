...
function getTime(date) {
  ...
  return mydate.toTimeString().slice(0, 8);
}
...
function getNextFriday(date) {
  ...
  } while (Days[event.getDay()] !== 'Friday');
  return event;
}
...
function getCountDaysInMonth(month, year) {
  const event = new Date(year, month - 1);
  let daysCount = 0;
  while (event.getMonth() === month - 1) {
    daysCount += 1;
    event.setDate(event.getDate() + 1);
  }
  return daysCount;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const startEvent = new Date(dateStart);
  const endEvent = new Date(dateEnd);
  endEvent.setDate(endEvent.getDate() + 1);
  return (endEvent.getTime() - startEvent.getTime()) / (1000 * 60 * 60 * 24);
}
...
function isDateInPeriod(date, period) {
  const startEvent = new Date(period.start).getTime();
  const endEvent = new Date(period.end).getTime();
  const currentEvent = new Date(date).getTime();
  return !!(currentEvent >= startEvent && currentEvent <= endEvent);
}
...
function getWeekNumberByDate(date) {
  ...
  while (Days[start.getDay()] !== 'Thursday') {
    start.setDate(start.getDate() + 1);
  }
  start.setDate(start.getDate() + 4);
  let weekNumber = 1;
  while (weekNumber < 12 * 4 + 1) {
...
function getNextFridayThe13th(date) {
  const event = new Date(date);
  const dayFriday = event.getDate() + (5 - event.getDay());
  event.setDate(dayFriday);
  while (event.getDate() !== 13) {
...
function getQuarter(date) {
  const quarters = [
    { date: new Date(date.getFullYear(), 11, 31, 23, 59, 59), quarter: 4 },
    { date: new Date(date.getFullYear(), 8, 30, 23, 59, 59), quarter: 3 },
    { date: new Date(date.getFullYear(), 5, 30, 23, 59, 59), quarter: 2 },
    { date: new Date(date.getFullYear(), 2, 31, 23, 59, 59), quarter: 1 },
  ];
  let quarter = 0;
  quarters.forEach((elem) => {
    if (date.getTime() < elem.date.getTime()) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const currentDay = new Date(startYear, startMonth - 1, startDay);
  ...
  while (currentDay <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      workSchedule.push(
        `${String(currentDay.getDate()).padStart(2, '0')}-${String(currentDay.getMonth() + 1).padStart(2, '0')}-${currentDay.getFullYear()}`
      );
